...
// UPDATED: This function contains all the JS logic and is wrapped in setTimeout as requested
export function initInteractiveMappingTool() {
    console.log("initInteractiveMappingTool: Function CALLED by parent.");

    // ADD THIS SETTIMEOUT WRAPPER
    setTimeout(() => {
        console.log("initInteractiveMappingTool: setTimeout EXECUTING now."); // New log

        const componentWrapper = document.getElementById('mapping-tool-wrapper');
        if (!componentWrapper) {
            console.error("initInteractiveMappingTool (in setTimeout): ERROR - Root wrapper 'mapping-tool-wrapper' NOT FOUND. Tool cannot initialize.");
            // Display error in the UI where the tool should be
            const parentMappingContainer = document.getElementById('mapping-component-container');
            if(parentMappingContainer) {
                parentMappingContainer.innerHTML = '<p class="text-red-500 text-center p-4">Error: Mapping tool could not load (wrapper not found).</p>';
            }
            return; 
        }
        console.log("initInteractiveMappingTool (in setTimeout): Root wrapper 'mapping-tool-wrapper' FOUND.");
        ...
        if (!mappingContainer || !nodesContainer || !canvas || !addNodeButton || !connectNodesButton || !teelParagraphsContainer || !addTeelParagraphButton || !removeTeelParagraphButton || !clearMappingDataButton || !mappingToolMessageEl) {
            console.error("initInteractiveMappingTool (in setTimeout): One or more essential child elements not found. Tool cannot initialize fully.");
            if(componentWrapper) {
                 componentWrapper.innerHTML = '<p class="text-red-500 text-center p-4">Error: Mapping tool components missing.</p>';
            } else {
                const parentMappingContainerFallback = document.getElementById('mapping-component-container');
                 if(parentMappingContainerFallback) parentMappingContainerFallback.innerHTML = '<p class="text-red-500 text-center p-4">Error: Mapping tool could not initialize (children not found).</p>';
            }
            return; 
        }
        console.log("initInteractiveMappingTool (in setTimeout): All child elements FOUND.");
        ...
        if (!ctx) {
            console.error("initInteractiveMappingTool (in setTimeout): Failed to get 2D context from canvas.");
            showMappingMessage("Error: Canvas rendering failed.", 0);
            return;
        }
        console.log("initInteractiveMappingTool (in setTimeout): Canvas context obtained.");
        ...
        // --- All the original helper functions, kept inside setTimeout ---
        ...
        const renderTeelParagraphs = () => {
            ...
            teelPlanData.forEach((paraData, index) => {
                ...
                paraBlock.innerHTML = `
                    ...
                    <div class="space-y-2">
                        <div><label class="block text-xs font-medium text-slate-300">Topic Sentence:</label><textarea data-index="${index}" data-field="topicSentence" class="teel-input w-full p-1 text-xs bg-slate-600 border border-slate-500 rounded">${paraData.topicSentence || ''}</textarea></div>
                        <div><label class="block text-xs font-medium text-slate-300">Evidence:</label><textarea data-index="${index}" data-field="evidence" class="teel-input w-full p-1 text-xs bg-slate-600 border border-slate-500 rounded">${paraData.evidence || ''}</textarea></div>
                        <div><label class="block text-xs font-medium text-slate-300">Explanation:</label><textarea data-index="${index}" data-field="explanation" class="teel-input w-full p-1 text-xs bg-slate-600 border border-slate-500 rounded">${paraData.explanation || ''}</textarea></div>
                        <div><label class="block text-xs font-medium text-slate-300">Link:</label><textarea data-index="${index}" data-field="link" class="teel-input w-full p-1 text-xs bg-slate-600 border border-slate-500 rounded">${paraData.link || ''}</textarea></div>
                    </div>
                `;
        ...
        // Re-define createNodeAndSave to use internalCreateNodeSetup if you separated them
        ...
        console.log("initInteractiveMappingTool (in setTimeout): Setup COMPLETE.");
    }, 0); // Defer execution
}
